// ESP32-C3 BLE NTP v0.12.4
// Changes from v0.12.3:
//   - info? BLE command -- version, settings, uptime, last sync, SSID
//   - resync:N BLE command -- set NTP re-sync interval in hours (1-168, default 100)
//   - Periodic NTP re-sync in loop() -- reconnects WiFi, syncs, disconnects
//   - lastSync timestamp tracked after every successful sync
// Carried forward from v0.12.3:
...
uint8_t       resyncHours  = 100;  // re-sync interval in hours, default 100
unsigned long lastSync     = 0;    // millis() timestamp of last successful NTP sync
unsigned long lastSyncUnix = 0;    // UTC unix time of last sync for info? display
...
// Sets lastSync and lastSyncUnix on success
// ---------------------------------------------------------------------------
void syncNTP() {
  ...
  if (myTZ.setLocation(tzName)) {
    ...
    ntpSynced     = true;
    lastSync      = millis();          // record millis for resync interval tracking
    lastSyncUnix  = UTC.now();         // record UTC time for info? display
  } else {
...
//   resync:N             set NTP re-sync interval in hours (1-168, default 100)
// ---------------------------------------------------------------------------
class RxCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pChar) {
    ...
    // -- info? ---------------------------------------------------------
    // Sends device info as multiple short notifies with 50ms gap between each
    } else if (value == "info?") {
      Serial.println(F("[BLE] info? -- sending device info"));
      bleNotifyLine("ver:0.12.4");
      ...
      if (lastSyncUnix > 0) {
        // Format last sync time from stored UTC unix timestamp
    ...
    // -- resync:N ------------------------------------------------------
    // Sets NTP re-sync interval in hours (1-168 = 1h to 7 days)
    // Saved to flash, applied immediately
...
void startBLE() {
  ...
  pCharTX->setValue("FingerClock v0.12.4 ready");
...
void setup() {
  ...
  hour12       = prefs.getBool("hour12",   false);  // default 24h
  flipped      = prefs.getBool("flip",     false);  // default USB bottom
  resyncHours  = prefs.getUChar("resync",  100);    // default 100h
  prefs.end();
...
void loop() {
  ...
  // Reconnects WiFi, syncs, disconnects -- only when BLE is not active
  // BLE and WiFi share the same radio -- cannot run simultaneously
